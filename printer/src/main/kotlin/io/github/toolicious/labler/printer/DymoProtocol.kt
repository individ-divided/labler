...
class DymoProtocol private constructor(tuning: ProtocolTuning) : PrinterProtocol {
    ...
    /**
     * Codes seen on the device: 0 after a finished job, 4 after one cancelled at the printer, and
     * 2 when it turns a job down because the tape ran out on the one before. Opening and closing
     * the lid clears that refusal, after which it prints into nothing without complaining.
     *
     * 7 never arrives. With the lid open the machine runs the motor and heats the head as if it
     * were printing, only the platen is not against the tape, so a missing cassette cannot be
     * told from a good print. The rest of the mapping is from the description, where 2 and 5 are
     * a general failure rather than the empty tape seen here.
     */
